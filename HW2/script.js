...
function cookie() {
    ...
    var greet_id = document.getElementById("cookie");
    var message = '<b>Good ';
    if (hour < 12)
        message += 'Morning ';
    else if (hour < 18)
    ...
    if (user!="")
    {
        message += user + "! Welcome again to the GMU CS Survey Page!</b><br>";
    }
    else {
        user = prompt("New user detected! Please enter your name","");
        if (user != "" && user != null) {
            setCookie("username", user);
            message += user + "! Welcome to GMU CS Survey !</b><br>";
        }
    }
